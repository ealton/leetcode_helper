...
class Solution:
    ...
    def test(self):
        # print('1' if [1] else '0')
        print(self.findLadders('cet', 'ism',
                               ["kid", "tag", "pup", "ail", "tun", "woo", "erg", "luz", "brr", "gay", "sip", "kay",
                                ...
                                "all", "pad", "hah", "hie", "aim"]))
        print(self.findLadders('aaaaa', 'ggggg',
    ...
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        for word in wordList:
            for i in range(len(word)):
                neighbor_graph[word[:i] + "*" + word[i + 1:]].append(word)

        if endWord not in wordList:
            return []

        visited1 = collections.defaultdict(list)
        q1 = collections.deque([beginWord])
        visited1[beginWord] = []

        visited2 = collections.defaultdict(list)
        ...
        def bfs(q, visited1, visited2, frombegin):
            level_visited = collections.defaultdict(list)
            for _ in range(len(q)):
                u = q.popleft()

                for i in range(len(u)):
                    for v in neighbor_graph[u[:i] + "*" + u[i + 1:]]:
                        if v in visited2:
                            paths1 = []
                            paths2 = []
